...
def load_users(session):
    ...
    for line in f:
        ...
        user_age = data[1]
        user_zipcode = data[4]
        # create new instance of the User class in model.py called my)new_user, set class attributes to variables defined in this function
...
def load_movies(session):
    ...
    with open(filename, 'rb') as csvfile:
        ...
        for row in openfile:
            ...
            movie_release = row[2]
            # convert data to datetime only when release date is not empty (otherwise, code will fail)
            if movie_release != "":
